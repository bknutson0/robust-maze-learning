...
def broyden(f, x0, threshold, eps=1e-12, stop_mode='abs', ls=False, layer_loss=True, layer_idx=[10], name='unknown'):
    ...
    protect_thres = (1e12 if stop_mode == 'abs' else 1e3) * seq_len
    new_objective = 1e8
    ...
    return {
        'result': lowest_xest.view(init_shape),
...
def anderson(f, x0, m=6, lam=1e-4, threshold=50, eps=1e-3, stop_mode='rel', beta=1.0, layer_idx=[10], **kwargs):
    alternative_mode = 'rel' if stop_mode == 'abs' else 'abs'
    """ Anderson acceleration for fixed point iteration. """
    if len(x0.shape) == 4:
        bsz, C, H, W = x0.shape
        X = torch.zeros(bsz, m, C * H * W, dtype=x0.dtype, device=x0.device)
        F = torch.zeros(bsz, m, C * H * W, dtype=x0.dtype, device=x0.device)
    elif len(x0.shape) == 3:
        bsz, d, L = x0.shape
    ...
    for k in range(2, threshold):
        ...
        if k in layer_idx:
            interm_vals.append(X[:, k % m].view_as(x0).clone().detach())
...
def analyze_broyden(res_info, err=None, judge=True, name='forward', training=True, save_err=True):
    ...
    if judge:
        return (
            nstep >= threshold
            or (nstep == 0 and (diff != diff or diff > eps))
            or prot_break
            or torch.isnan(res_est).any()
        )
