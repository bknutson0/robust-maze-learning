...
def deadend_fill(inputs: torch.Tensor) -> torch.Tensor:
    ...
    for grid in pixel_grid:
        ...
        # Backfill the maze
        deadend_filled_maze = deadend_fill_maze(targeted_lattice_maze)  # type: ignore
...
def deadend_fill_maze(maze: TargetedLatticeMaze) -> TargetedLatticeMaze:
    """Iteratively remove nodes with only one neighbor, not connected to start or end until no more can be removed."""
...
def _should_remove_node(maze: TargetedLatticeMaze, pos: tuple[int, int]) -> bool:
...
def _remove_node(connection_list: np.ndarray[np.bool_, np.dtype[np.bool_]], pos: tuple[int, int]) -> None:
